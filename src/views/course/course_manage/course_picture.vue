...
<script>
  import { deleteCoursePic, addCoursePic, findCoursePicList } from "@/api/course/course";
  export default {
    data() {
      return {
        picmax:1,//最大上传文件的数量
    ...
    methods: {
      ...
      handleRemove(file, fileList) {
        ...
        return new Promise((resolve,rejct)=>{
          deleteCoursePic(this.courseid).then(res=>{
            if(res.success){
...
//        alert('上传成功')
        ...
        if(response.success){
          ...
          let fileId = response.path;
          addCoursePic(this.courseid,fileId).then(res=>{
              if(res.success){
    ...
    mounted(){
      ...
      this.userId = '1'
      this.uploadUrl = `cms/filesystem/upload?userId=${this.userId}&courseId=${this.courseid}`
      this.uploadval.filetag = this.$route.params.courseid;
      //查询课程
      findCoursePicList(this.courseid).then(res=>{
          if(res && res.pic){
              let imgUrl = 'http://'+res.pic;
