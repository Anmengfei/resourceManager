...
<script>
  ...
  import { updateCourseMarket, getCourseMarketById } from "@/api/course/course";
  export default {
    data() {
      return {
        active: 1,
        ...
        courseid:'',
        chargeList:[
          { "sdName" : "免费", "sdId" : "203001", "sd_status" : "1" },
          { "sdName" : "收费", "sdId" : "203002", "sd_status" : "1" }
        ],
        validList:[
        ...
        this.$refs.courseMarketForm.validate((valid) => {
          if (valid) {
            this.$confirm('确认提交吗？', '提示', {}).then(() => {
              updateCourseMarket(this.courseid,this.courseMarketForm).then((res) => {
    ...
    mounted(){
      ...
      //获取课程营销信息
      getCourseMarketById(this.courseid).then((res) => {
         console.log(res);
        if(res && res.id){
